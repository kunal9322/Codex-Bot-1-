...
#    Special credits to  https://t.me/little_little_hackur
import re
...
@Codex.on_message(filters.command("chatbot") & ~filters.edited & ~filters.bot)
@admins_only
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("ᴄᴏᴅᴇx ᴀɪ ᴀʟʀᴇᴀᴅʏ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ɪɴ ᴛʜɪs ᴄʜᴀᴛ")
            return
        await lel.edit(
            f"ᴄᴏᴅᴇx ᴀɪ sᴜᴄᴄᴇssғᴜʟʟʏ ᴀᴅᴅᴇᴅ ғᴏʀ ᴜsᴇʀs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("ᴄᴏᴅᴇx ᴀɪ ᴡᴀs ɴᴏᴛ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ɪɴ ᴛʜɪs ᴄʜᴀᴛ")
            return
        await lel.edit(
            f"ᴄᴏᴅᴇx ᴀɪ sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇᴀᴄᴛɪᴠᴀᴛᴇᴅ ғᴏʀ ᴜsᴇʀs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ {message.chat.id}"
        )

    elif status == "EN" or status == "en" or status == "english":
        if not chat_id in en_chats:
            en_chats.append(chat_id)
            await message.reply_text("ᴇɴɢʟɪsʜ ᴀɪ ᴄʜᴀᴛ ᴇɴᴀʙʟᴇᴅ!")
            return
        await message.reply_text("ᴀɪ ᴄʜᴀᴛ ɪs ᴀʟʀᴇᴀᴅʏ ᴅɪsᴀʙʟᴇᴅ.")
        message.continue_propagation()
    else:
        await message.reply_text(
            "I only recognize `/chatbot on` and /chatbot `off only`"
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        result = result.replace('{"cnt":"', "")
        result = result.replace('"}', "")
        result = result.replace("Aco", "codex")
        result = result.replace("<a href=\\", "<a href =")
        result = result.replace("<\/a>", "</a>")
        pro = result
        if not "en" in lan and not lan == "":
...
@Codex.on_message(filters.text & filters.private & ~filters.reply & ~filters.bot)
...
@Codex.on_message(
    filters.regex("codex|Codex|CoDexBoTsupport|hello|hi")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
